...
void loginPage(char &login)
{
    ...
    cout << "\t\t -----------------------------------------------------------------------------" << endl;
    cout << "\t\t | *If you already have an account then press \'L\' for login your account :   |" << endl;
...
account createNewAccount(account *&accs, int &noOfAccounts)
{
    ...
    {
        spaces2();
        cout << "\t\t Do your first movement by depositing at least Rs.200:";
        ...
        {
            cout << "\t\t Error : Low balance you must have deposit at least Rs.200 :";
            newAccount.setMovementsValue();
        }
...
void requestLoan(account *accounts, int &noOfAccounts, account &currentUser, char login, bool flag)
{
    ...
    if (currentUser.getTotalBalance() >= 500)
    {
        ...
        Sleep(3 * 1000);
        system("cls");
        cout << "\t\t\t You request is approved ,You got Rs." << amount << " of loan from Ankur Bank Limited (ABL) " << endl;
    ...
    {
        system("cls");
        cout << "\t\t Low Balance ! For requesting Loan you must have at least Rs.500 in your account. " << endl;
        secondPage(currentUser, login, accounts, noOfAccounts, flag);
    }
}
...
void transictionProcess(account *accounts, int &noOfAccounts, account &currentUser, char login, bool flag)
{
    ...
    {
        cout << "Sorry Sir ! You cannot transfer more than Rs." << currentUser.getTotalBalance() - 200 << ", At least your bank account has Rs.200 in it." << endl;
    ...
    do
    {
        ...
        if (accountExist == true && selfTransfer == false)
        {
            ...
            Sleep(3 * 1000);
            system("cls");
            cout << "\t\t\t SUCCESSFULLY TRANSFERED ,You transfered Rs." << amountTransfer << " to the account  " << usernameTransfer << endl;
...
void deleteAccount(account *accounts, int &noOfAccounts, account &currentUser, char login, bool flag, bool &exit)
{
    ...
    do
    {
        ...
        {
            cout << "\t\t You entered the worng username or pincode : Try Again : " << endl;
        }
    } while (found == false);
}
